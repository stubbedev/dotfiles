...
''
  ...
  get_latest_nvim_version() {
    ${pkgs.curl}/bin/curl -s https://api.github.com/repos/neovim/neovim/releases/latest | ${pkgs.jq}/bin/jq -r '.tag_name | ltrimstr("v")'
  }
  ...
  eval "\"${pkgs.bun}/bin/bun\" pm trust --all $REDIRECT_SUFFIX" || true
  ...
  if check_nvim_needs_update; then
    install_nvim
  fi
''
