...
let
  ...
  wrapWith =
    ...
    let
      programPath = if exeName == null then lib.getExe program else lib.getExe' program exeName;
...
in
{
  ...
  gfxNameWith =
    name: prelude: program:
    wrapWith { inherit program name prelude; };
  gfxExe = exeName: program: wrapWith { inherit program exeName; };
  gfxNameExe =
    name: exeName: program:
    wrapWith { inherit program name exeName; };
