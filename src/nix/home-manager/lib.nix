...

{
  ...
  loadPackagesFromDir = dir: args:
    ...
    in
    lib.flatten (map loadFile (lib.attrNames nixFiles));
      ...
      let
        nixFiles = lib.filterAttrs
          (name: type:
            ...
            != "default.nix" && name != "metadata.nix")
          (builtins.readDir dir);
        tryLoadFile = name:
          let path = dir + "/${name}";
          in if builtins.pathExists path then [ (import path args) ] else [ ];
      in
      lib.flatten (map tryLoadFile (lib.attrNames nixFiles))
    else
      [ ];
  ...
  loadModulesFromDirFiltered = dir: filterFn:
    ...
    in
    map (name: dir + "/${name}") filteredNames;
  ...
  loadVpnScripts = vpnDir:
    let
      ...
      allEntries =
        lib.flatten (map createScriptEntries (lib.attrNames vpnProviders));
    in
