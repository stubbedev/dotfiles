{ inputs, ... }:
{
  flake.modules.homeManager.context =
    ...
    let
      ...
      vpnConfigs = if config.features.vpn then homeLib.loadVpnConfigs ../../../../vpn else { };
      vpnScripts = if config.features.vpn then homeLib.loadVpnScripts ../../../../vpn else { };
    in
    {
      _module.args = {
        inherit
          ...
          vpnScripts
          ;
        inherit (inputs) hyprland hy3 fenix tmux-stubbe opencode;
