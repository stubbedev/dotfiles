# Shared Chrome enterprise-policy body.
#
# Lives in lib/ (not modules/) so import-tree does not load it as a flake
# module; it is imported directly by both Chrome policy modules so the
# policy has a single source:
#   modules/activation/privileged/setup-chrome-policy.nix  (non-NixOS)
