_: {
  flake.modules.homeManager.homeXdgHypr =
    {
      config,
      constants,
      lib,
    ...
    let
      ...
      # Catppuccin Mocha palette (hex, no #). Shared source of truth in
      # lib/colors.nix (homeLib.catppuccinMocha); generates the Lua colors
      # returned from nix.lua (used by hyprland.lua).
      palette = homeLib.catppuccinMocha;
