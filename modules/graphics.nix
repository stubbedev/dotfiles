...
in
{
  flake.modules.nixos.graphics =
    ...
    }:
    {
      ...
      boot = {
        ...
        # healing — see `kernelModules` below.
        extraModulePackages = [
  ...
  flake.modules.homeManager.graphics =
    ...
    lib.mkIf config.features.desktop {
      # Point KDE Plasma's Wayland session at our nixGL-wrapped Xwayland, so
      # X11 apps (Steam) get GLX with the NVIDIA driver. Inert on a
      # Hyprland-only host, kept because a Plasma session is the fallback when
      # debugging a compositor problem.
      xdg.configFile.kwinrc = {
        text = lib.generators.toINI { } {
          Xwayland = {
            Scale = 1;
            ServerPath = "${config.stubbe.paths.nixBin}/Xwayland";
